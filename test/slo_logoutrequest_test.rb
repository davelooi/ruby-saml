...
class RubySamlFTest < Minitest::Test
  ...
  describe "SloLogoutrequest" do
    ...
    let(:logout_request) { OneLogin::RubySamlF::SloLogoutrequest.new(logout_request_document) }
    let(:invalid_logout_request) { OneLogin::RubySamlF::SloLogoutrequest.new(invalid_logout_request_document) }
    ...
    describe "initiator" do
      it "raise an exception when logout request is initialized with nil" do
        assert_raises(ArgumentError) { OneLogin::RubySamlF::SloLogoutrequest.new(nil) }
      end
    end

    describe "#is_valid?" do
      it "return false when logout request is initialized with blank data" do
        logout_request_blank = OneLogin::RubySamlF::SloLogoutrequest.new('')
      ...
      it "raise error for invalid xml" do
        ...
        assert_raises(OneLogin::RubySamlF::ValidationError) { invalid_logout_request.is_valid? }
      end
    end
   ...
   describe "#not_on_or_after" do
    ...
    describe '#session_indexes' do
      ...
      it "return an Array with one SessionIndex" do
        logout_request_with_session_index = OneLogin::RubySamlF::SloLogoutrequest.new(logout_request_xml_with_session_index)
    ...
    describe "#validate_not_on_or_after" do
      ...
      it "raise when the logout request has an invalid NotOnOrAfter" do
        ...
        assert_raises(OneLogin::RubySamlF::ValidationError, "Current time is on or after NotOnOrAfter") do
    ...
    describe "#validate_request_state" do
      ...
      it "raise error for invalid xml" do
        ...
        assert_raises(OneLogin::RubySamlF::ValidationError, "Blank logout request") do
    ...
    describe "#validate_structure" do
      ...
      it "raise when encountering a Logout Request bad formatted" do
        invalid_logout_request.soft = false
        assert_raises(OneLogin::RubySamlF::ValidationError, "Element '{urn:oasis:names:tc:SAML:2.0:assertion}Issuer': This element is not expected") do
    ...
    describe "#validate_issuer" do
      ...
      it "raise when the issuer of the Logout Request does not match the IdP entityId" do
        logout_request.settings.idp_entity_id = 'http://idp.example.com/invalid'
        logout_request.soft = false
        assert_raises(OneLogin::RubySamlF::ValidationError, "Doesn't match the issuer, expected: <#{logout_request.settings.idp_entity_id}>, but was: <https://app.onelogin.com/saml/metadata/SOMEACCOUNT>") do
    ...
    describe "#validate_signature" do
      ...
      it "return true when valid RSA_SHA1 Signature" do
        ...
        params = OneLogin::RubySamlF::Logoutrequest.new.create_params(settings, :RelayState => 'http://example.com')
        params['RelayState'] = params[:RelayState]
        options = {}
        options[:get_params] = params
        logout_request_sign_test = OneLogin::RubySamlF::SloLogoutrequest.new(params['SAMLRequest'], options)
      ...
      it "raise when invalid RSA_SHA1 Signature" do
        ...
        logout_request_sign_test = OneLogin::RubySamlF::SloLogoutrequest.new(params['SAMLRequest'], options)
        assert_raises(OneLogin::RubySamlF::ValidationError, "Invalid Signature on Logout Request") do
