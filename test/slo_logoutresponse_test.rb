...
class SloLogoutresponseTest < Minitest::Test
  ...
  describe "SloLogoutresponse" do
    ...
    it "create the deflated SAMLResponse URL parameter" do
      unauth_url = OneLogin::RubySamlF::SloLogoutresponse.new.create(settings, logout_request.id)
    ...
    it "support additional params" do
      unauth_url = OneLogin::RubySamlF::SloLogoutresponse.new.create(settings, logout_request.id, nil, { :hello => nil })
      assert_match /&hello=$/, unauth_url

      unauth_url = OneLogin::RubySamlF::SloLogoutresponse.new.create(settings, logout_request.id, nil, { :foo => "bar" })
      assert_match /&foo=bar$/, unauth_url

      unauth_url = OneLogin::RubySamlF::SloLogoutresponse.new.create(settings, logout_request.id, nil, { :RelayState => "http://idp.example.com" })
    ...
    it "set a custom successful logout message on the response" do
      unauth_url = OneLogin::RubySamlF::SloLogoutresponse.new.create(settings, logout_request.id, "Custom Logout Message")
    ...
    describe "when the settings indicate to sign (embedded) logout response" do
      ...
      it "create a signed logout response" do
        logout_request.settings = settings
        params = OneLogin::RubySamlF::SloLogoutresponse.new.create_params(settings, logout_request.id, "Custom Logout Message")
    ...
    describe "#create_params when the settings indicate to sign the logout response" do
      ...
      it "create a signature parameter with RSA_SHA1 and validate it" do
        ...
        params = OneLogin::RubySamlF::SloLogoutresponse.new.create_params(settings, logout_request.id, "Custom Logout Message", :RelayState => 'http://example.com')
