...
class RubySamlFTest < Test::Unit::TestCase
  ...
  context "Response" do
    ...
    should "be able to parse a document which contains ampersands" do
      XMLSecurityF::SignedDocument.any_instance.stubs(:digests_match?).returns(true)
      OneLogin::RubySamlF::Response.any_instance.stubs(:validate_conditions).returns(true)

      response = OneLogin::RubySamlF::Response.new(ampersands_response)
    ...
    should "adapt namespace" do
      response = OneLogin::RubySamlF::Response.new(response_document)
      assert !response.name_id.nil?
      response = OneLogin::RubySamlF::Response.new(response_document_2)
      assert !response.name_id.nil?
      response = OneLogin::RubySamlF::Response.new(response_document_3)
      assert !response.name_id.nil?
    end

    should "default to raw input when a response is not Base64 encoded" do
      decoded  = Base64.decode64(response_document_2)
      response = OneLogin::RubySamlF::Response.new(decoded)
      assert response.document
    end

    context "Assertion" do
      should "only retreive an assertion with an ID that matches the signature's reference URI" do
        response = OneLogin::RubySamlF::Response.new(wrapped_response_2)
    ...
    context "#is_valid?" do
      should "return false when response is initialized with blank data" do
        response = OneLogin::RubySamlF::Response.new('')
      ...
      should "return true when the response is initialized with valid data" do
        response = OneLogin::RubySamlF::Response.new(response_document_4)
        response.stubs(:conditions).returns(nil)
        assert !response.is_valid?
        settings = OneLogin::RubySamlF::Settings.new
        assert !response.is_valid?
      ...
      should "support dynamic namespace resolution on signature elements" do
        response = OneLogin::RubySamlF::Response.new(fixture("no_signature_ns.xml"))
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlF::Settings.new
        response.settings = settings
        settings.idp_cert_fingerprint = "28:74:9B:E8:1F:E8:10:9C:A8:7C:A9:C3:E3:C5:01:6C:92:1C:B4:BA"
        XMLSecurityF::SignedDocument.any_instance.expects(:validate_signature).returns(true)
        assert response.validate!
      end

      should "validate ADFS assertions" do
        response = OneLogin::RubySamlF::Response.new(fixture(:adfs_response_sha256))
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlF::Settings.new
        settings.idp_cert_fingerprint = "28:74:9B:E8:1F:E8:10:9C:A8:7C:A9:C3:E3:C5:01:6C:92:1C:B4:BA"
        response.settings = settings
      ...
      should "validate the digest" do
        response = OneLogin::RubySamlF::Response.new(r1_response_document_6)
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlF::Settings.new
      ...
      should "validate SAML 2.0 XML structure" do
        ...
        response = OneLogin::RubySamlF::Response.new(Base64.encode64(resp_xml))
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlF::Settings.new
        settings.idp_cert_fingerprint = signature_fingerprint_1
        response.settings = settings
        assert_raises(OneLogin::RubySamlF::ValidationError, 'Digest mismatch'){ response.validate! }
      end
    end
    ...
    context "#name_id" do
      ...
      should "be extractable from an OpenSAML response" do
        response = OneLogin::RubySamlF::Response.new(fixture(:open_saml))
    ...
    context "#check_conditions" do
      should "check time conditions" do
        ...
        response = OneLogin::RubySamlF::Response.new(response_document_6)
        assert response.send(:validate_conditions, true)
        time     = Time.parse("2011-06-14T18:25:01.516Z")
        Time.stubs(:now).returns(time)
        response = OneLogin::RubySamlF::Response.new(response_document_5)
        assert response.send(:validate_conditions, true)
      end
      ...
      should "optionally allow for clock drift" do
        ...
        response = OneLogin::RubySamlF::Response.new(response_document_5, :allowed_clock_drift => 0.515)
        assert !response.send(:validate_conditions, true)

        Time.stubs(:now).returns(Time.parse("2011-06-14T18:21:01Z"))
        response = OneLogin::RubySamlF::Response.new(response_document_5, :allowed_clock_drift => 0.516)
        assert response.send(:validate_conditions, true)
      end
    end
    ...
    context '#xpath_first_from_signed_assertion' do
      should 'not allow arbitrary code execution' do
        ...
        response = OneLogin::RubySamlF::Response.new(malicious_response_document)
