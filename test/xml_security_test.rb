...
class XmlSecurityTest < Minitest::Test
  ...
  describe "XmlSecurity" do
    ...
    it "should raise Fingerprint mismatch" do
      exception = assert_raises(OneLogin::RubySamlF::ValidationError) do
  ...
  describe "Fingerprint Algorithms" do
    let(:response_fingerprint_test) { OneLogin::RubySamlF::Response.new(fixture(:adfs_response_sha1, false)) }
  ...
  describe "XmlSecurity::SignedDocument" do
    ...
    describe "XMLSecurity::DSIG" do
      ...
      it "sign an AuthNRequest" do
        request = OneLogin::RubySamlF::Authrequest.new.create_authentication_xml_doc(settings)
        ...
        request2 = OneLogin::RubySamlF::Authrequest.new.create_authentication_xml_doc(settings)
      ...
      it "sign a LogoutRequest" do
        logout_request = OneLogin::RubySamlF::Logoutrequest.new.create_logout_request_xml_doc(settings)
        logout_request.sign_document(ruby_saml_key, ruby_saml_cert)
        # verify our signature
        signed_doc = XMLSecurity::SignedDocument.new(logout_request.to_s)
        assert signed_doc.validate_document(ruby_saml_cert_fingerprint, false)

        logout_request2 = OneLogin::RubySamlF::Logoutrequest.new.create_logout_request_xml_doc(settings)
      ...
      it "sign a LogoutResponse" do
        logout_response = OneLogin::RubySamlF::SloLogoutresponse.new.create_logout_response_xml_doc(settings, 'request_id_example', "Custom Logout Message")
        logout_response.sign_document(ruby_saml_key, ruby_saml_cert)
        # verify our signature
        signed_doc = XMLSecurity::SignedDocument.new(logout_response.to_s)
        assert signed_doc.validate_document(ruby_saml_cert_fingerprint, false)

        logout_response2 = OneLogin::RubySamlF::SloLogoutresponse.new.create_logout_response_xml_doc(settings, 'request_id_example', "Custom Logout Message")
    ...
    describe "StarfieldTMS" do
      let (:response) { OneLogin::RubySamlF::Response.new(fixture(:starfield_response)) }

      before do
        response.settings = OneLogin::RubySamlF::Settings.new( :idp_cert_fingerprint => "8D:BA:53:8E:A3:B6:F9:F1:69:6C:BB:D9:D8:BD:41:B3:AC:4F:9D:4D")
      end
